...
///
/// For a prebuilt package — a `.webc`, or the `.webcm` sidecar naming one —
/// the returned path is the `.webc` and the manifest is reconstructed from its
...
fn get_manifest_from_webc_file(path: &Path, webcm_path: Option<&Path>) -> anyhow::Result<Manifest> {
    ...
    if let Some(expected) = webcm.as_ref().and_then(|w| w.package.hash.as_ref()) {
...
/// The package hash (sha256) of the file at `path`, computed by streaming.
pub(super) fn sha256_of_file(path: &Path) -> anyhow::Result<PackageHash> {
    use sha2::Digest;
    use std::io::BufRead;
    ...
    Ok(PackageHash::from_sha256_bytes(hasher.finalize().into()))
}
