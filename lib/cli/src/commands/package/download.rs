...
impl PackageDownload {
    pub(crate) fn execute(&self) -> Result<(), anyhow::Error> {
        ...
        // Pair the (anonymous) webc with a sidecar recording its identity. The
        // hash is the registry's, not one we compute: if the download was
...
mod tests {
    ...
    fn test_cmd_package_download() {
        ...
        assert_eq!(
            webcm.package.hash.unwrap(),
            super::super::common::sha256_of_file(&out_path).unwrap()
        );
    }
}
