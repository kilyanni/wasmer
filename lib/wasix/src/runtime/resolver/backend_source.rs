...
fn matching_package_summaries(
    ...
    let exact_pin = exact_pinned_version(version_constraint);
    ...
    for pkg_version in versions {
        ...
        if pkg_version.yanked_at.is_some() {
            if exact_pin.as_ref() == Some(&version) {
                // Warn on stderr because the CLI gates `tracing` off at the
                // default verbosity.
                match &pkg_version.yank_reason {
                    Some(reason) => eprintln!(
                        "warning: {namespace}/{package_name}@{version} has been yanked: {reason}"
                    ),
                    None => eprintln!(
                        "warning: {namespace}/{package_name}@{version} has been yanked"
                    ),
                }
            } else {
                tracing::debug!(
