...
impl InMemorySource {
    ...
    /// tree of them.
    ///
    /// A `.webcm` sidecar (same basename as its `.webc`) is the canonical
    /// package identity: a built webc is anonymous (name stripped on build),
    /// and the sidecar restores it. Webcs without a sidecar keep the id from
    /// their own manifest. An invalid sidecar — unparseable, missing its
    /// paired webc, or a hash mismatch — is an error, never silently skipped.
